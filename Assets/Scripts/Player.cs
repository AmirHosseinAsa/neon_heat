...
public class Player : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (deathStart + 2 < Time.time && readyToDie)
        {
            ...
            StartCoroutine(Viberation(.65f, .65f));
        }
        ...
        if (Input.GetKey(KeyCode.D) || Input.GetAxis("Horizontal") == 1 || Input.GetKey(KeyCode.RightArrow))
        {
        ...
        if (Input.GetKey(KeyCode.A) || Input.GetAxis("Horizontal") == -1 || Input.GetKey(KeyCode.LeftArrow))
        {
            currentHorizontalSpeed = Mathf.Lerp(currentHorizontalSpeed, maxHorizontalSpeed + bonusHorizontalSpeed + boostHorizontalSpeed, Time.deltaTime / 0.2f);
        }

        //If none of the buttons are pressed, lerp to 0 on horizontal speed.
        if (!Input.GetKey(KeyCode.D) && !Input.GetKey(KeyCode.A) && Input.GetAxis("Horizontal") != -1 && Input.GetAxis("Horizontal") != 1 && !Input.GetKey(KeyCode.LeftArrow) && !Input.GetKey(KeyCode.RightArrow))
        {
        ...
        if (rb.velocity.z > minimumSpeed)
        { //Remember, the minimum speed is negative.
    ...
    float CalculateBonusHorizontalSpeed()
    { //Change this to take in 2 ranges and a multiplier later.
    ...
    void OnTriggerEnter(Collider other)
    {
        ...
        {
            StartCoroutine(Viberation(.15f, .35f));
        ...
        if (other.tag == "Portal")
        {
            StartCoroutine(Viberation(.15f, .15f));
            if (!desertMode)
            {
        ...
        {
            StartCoroutine(Viberation(.9f, .7f));
            StartDropDown();
            GameObject.Find("FadeOut").GetComponent<FadeOut>().StartFadeOut();
        }
